...
/** Article detail URL. Content categories organize MDX but do not appear in public URLs. */
export function detailPath(_category: string, slug: string, locale: Locale): string {
  const publicSlug = slug === '10-update' ? '1.0-update' : slug;
  return localizePath(`/${publicSlug}`, locale);
}
